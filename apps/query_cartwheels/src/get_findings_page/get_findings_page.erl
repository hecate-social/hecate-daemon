...
execute(#{cartwheel_id := _CartwheelId} = Filters) ->
    ...
    Sql = "SELECT finding_id, cartwheel_id, category, title, content, "
...
build_where(Filters, _Clauses, _Params, N) ->
    %% cartwheel_id is always required
...
row_to_map({FindingId, CartwheelId, Category, Title, Content,
            Priority, RecordedAt}) ->
    #{
