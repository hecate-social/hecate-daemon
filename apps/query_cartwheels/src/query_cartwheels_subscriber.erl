%%% @doc Event subscriber for query_cartwheels projections.
%%% Subscribes to manage_cartwheels_store events and cartwheels to SQLite.
-module(query_cartwheels_subscriber).
...
init([]) ->
    SubIds = lists:filtermap(fun(EventType) ->
        case reckon_evoq_adapter:subscribe(
            manage_cartwheels_store,
            event_type,
            EventType,
            <<"query_cartwheels_subscriber_", EventType/binary>>,
    ...
    io:format("~n[query_cartwheels_subscriber] Subscribed to Cartwheel events (~p types)~n", [length(SubIds)]),
...
terminate(_Reason, #state{subscription_ids = SubIds}) ->
    lists:foreach(fun(SubId) ->
        reckon_evoq_adapter:unsubscribe(manage_cartwheels_store, SubId)
    end, SubIds).

%% Internal

project_event(#{event_type := <<"cartwheel_initiated_v1">>} = E, State) ->
    safe_project(fun() -> cartwheel_initiated_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"discovery_started_v1">>} = E, State) ->
    safe_project(fun() -> discovery_started_v1_to_cartwheels:project(E) end, State);
...
project_event(#{event_type := <<"discovery_completed_v1">>} = E, State) ->
    safe_project(fun() -> discovery_completed_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"phase_transitioned_v1">>} = E, State) ->
    safe_project(fun() -> phase_transitioned_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"architecture_started_v1">>} = E, State) ->
    safe_project(fun() -> architecture_started_v1_to_cartwheels:project(E) end, State);
...
project_event(#{event_type := <<"plan_approved_v1">>} = E, State) ->
    safe_project(fun() -> plan_approved_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"architecture_completed_v1">>} = E, State) ->
    safe_project(fun() -> architecture_completed_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"testing_started_v1">>} = E, State) ->
    safe_project(fun() -> testing_started_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"skeleton_created_v1">>} = E, State) ->
    safe_project(fun() -> skeleton_created_v1_to_cartwheels:project(E) end, State);
...
project_event(#{event_type := <<"testing_completed_v1">>} = E, State) ->
    safe_project(fun() -> testing_completed_v1_to_cartwheels:project(E) end, State);
project_event(#{event_type := <<"deployment_started_v1">>} = E, State) ->
    safe_project(fun() -> deployment_started_v1_to_cartwheels:project(E) end, State);
...
project_event(#{event_type := <<"deployment_completed_v1">>} = E, State) ->
    safe_project(fun() -> deployment_completed_v1_to_cartwheels:project(E) end, State);
