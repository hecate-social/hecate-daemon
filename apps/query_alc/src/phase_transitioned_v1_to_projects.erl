%%% @doc Projection: phase_transitioned_v1 -> projects table (UPDATE current_phase + status)
-module(phase_transitioned_v1_to_projects).

-export([project/1]).

-spec project(map()) -> ok | {error, term()}.
project(#{project_id := PId, to_phase := ToPhase} = E) ->
    StatusFlag = phase_to_status_flag(ToPhase),
    Sql = "UPDATE projects SET current_phase = ?1, status = status | ?2, "
          "phase_started_at = ?3 WHERE project_id = ?4",
