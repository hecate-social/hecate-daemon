%%% @doc Projection: spoke_inventoried_v1 -> spoke_inventory table + projects counter
...
project(#{spoke_id := SId, project_id := PId, spoke_name := Name, spoke_type := Type, dossier_id := DId} = E) ->
    InsertSql = "INSERT OR REPLACE INTO spoke_inventory "
                "(spoke_id, project_id, spoke_name, spoke_type, priority, dossier_id, description, inventoried_at) "
    ...
    CountSql = "UPDATE projects SET spoke_count = spoke_count + 1 "
               "WHERE project_id = ?1",
