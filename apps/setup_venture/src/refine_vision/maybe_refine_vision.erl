%%% @doc maybe_refine_vision handler
%%% Business logic for refining venture vision during setup.
%%% Validates the command and dispatches via evoq.
-module(maybe_refine_vision).

-include_lib("evoq/include/evoq.hrl").

-export([handle/1, dispatch/1]).


%% @doc Handle refine_vision_v1 command (business logic only)
-spec handle(refine_vision_v1:refine_vision_v1()) ->
    {ok, [venture_vision_refined_v1:venture_vision_refined_v1()]} | {error, term()}.
handle(Cmd) ->
    VentureId = refine_vision_v1:get_venture_id(Cmd),
    case validate_command(VentureId) of
        ok ->
            Event = create_event(Cmd),
            {ok, [Event]};
        {error, Reason} ->
            {error, Reason}
    end.

%% @doc Dispatch command via evoq (persists to ReckonDB, enforces aggregate guards)
-spec dispatch(refine_vision_v1:refine_vision_v1()) ->
    {ok, non_neg_integer(), [map()]} | {error, term()}.
dispatch(Cmd) ->
    VentureId = refine_vision_v1:get_venture_id(Cmd),
    Timestamp = erlang:system_time(millisecond),

    EvoqCmd = #evoq_command{
        command_type = refine_vision,
        aggregate_type = setup_aggregate,
        aggregate_id = VentureId,
        payload = refine_vision_v1:to_map(Cmd),
        metadata = #{timestamp => Timestamp, aggregate_type => setup_aggregate},
        causation_id = undefined,
        correlation_id = undefined
    },

    Opts = #{
        store_id => setup_venture_store,
        adapter => reckon_evoq_adapter,
        consistency => eventual
    },

    evoq_dispatcher:dispatch(EvoqCmd, Opts).

%% Internal

validate_command(VentureId) when is_binary(VentureId), byte_size(VentureId) > 0 ->
    ok;
validate_command(_) ->
    {error, invalid_venture_id}.

create_event(Cmd) ->
    venture_vision_refined_v1:new(#{
        venture_id => refine_vision_v1:get_venture_id(Cmd),
        brief => refine_vision_v1:get_brief(Cmd),
        repos => refine_vision_v1:get_repos(Cmd),
        skills => refine_vision_v1:get_skills(Cmd),
        context_map => refine_vision_v1:get_context_map(Cmd),
        refined_by => refine_vision_v1:get_refined_by(Cmd)
    }).
