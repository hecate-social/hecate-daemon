%%% @doc SQLite store for launcher read models.
%%%
%%% Tables: launcher_entries, launcher_groups
%%% Populated by event projections (entry_registered_v1, etc.).
%%% @end
...
init([]) ->
    DbPath = shared_paths:sqlite_path("launcher.db"),
...
step_until_done(Stmt) ->
    case esqlite3:step(Stmt) of
        '$done' -> ok;
        {error, Code} ->
            logger:error("[project_launcher_store] SQLite step error: ~p", [Code]),
            {error, Code};
