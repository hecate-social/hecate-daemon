...
handle_post(Req0, _State) ->
    ...
    case DivisionId of
        ...
        _ -> case hecate_api_utils:read_json_body(Req0) of
...
do_generate(DivisionId, Params, Req) ->
    CmdParams = #{division_id => DivisionId, module_name => hecate_api_utils:get_field(module_name, Params),
        module_type => hecate_api_utils:get_field(module_type, Params), path => hecate_api_utils:get_field(path, Params)},
...
dispatch(Cmd, Req) ->
    case maybe_generate_module:dispatch(Cmd) of
        {ok, _Version, EventMaps} ->
            ...
            hecate_api_utils:json_ok(201, #{division_id => generate_module_v1:get_division_id(Cmd),
                module_name => generate_module_v1:get_module_name(Cmd), events => EventMaps}, Req);
        {error, Reason} -> hecate_api_utils:bad_request(Reason, Req)
    end.

emit_to_pg(EventMaps) -> lists:foreach(fun(E) -> module_generated_v1_to_pg:emit(E) end, EventMaps).
