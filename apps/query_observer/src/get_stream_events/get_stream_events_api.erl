%%% @doc GET /api/observer/stores/:store_id/streams/:stream_id — Stream events.
%%%
%%% Paginated events from a specific stream.
%%% Query params: offset=0, limit=50, direction=forward|backward
%%% @end
-module(get_stream_events_api).

-export([init/2, routes/0]).

routes() -> [{"/api/observer/stores/:store_id/streams/:stream_id", ?MODULE, []}].
...
handle_get(Req0, _State) ->
    ...
    Direction = qs_direction(QS),

    StoreId = binary_to_existing_atom(StoreIdBin),
    case evoq_event_store:read(StoreId, StreamId, Offset, Limit, Direction) of
        {ok, Events} ->
...
qs_direction(QS) ->
    case proplists:get_value(<<"direction">>, QS) of
        <<"backward">> -> backward;
        _ -> forward
    end.
