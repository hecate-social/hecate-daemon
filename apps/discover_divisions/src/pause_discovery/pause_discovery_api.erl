    ...
    case pause_discovery_v1:new(#{venture_id => VentureId, reason => Reason}) of
        {ok, Cmd} ->
            case maybe_pause_discovery:dispatch(Cmd) of
                {ok, Version, Events} ->
                    lists:foreach(fun(E) -> discovery_paused_v1_to_pg:emit(E) end, Events),
                    hecate_api_utils:json_reply(200, #{venture_id => VentureId, paused => true,
                        version => Version, events => Events}, Req0);
                {error, Reason2} ->
                    hecate_api_utils:json_error(422, Reason2, Req0)
            end;
        {error, Reason3} ->
            hecate_api_utils:json_error(400, Reason3, Req0)
    end.
