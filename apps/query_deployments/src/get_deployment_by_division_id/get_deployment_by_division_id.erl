%%% @doc Query: get deployment state for a division.
-module(get_deployment_by_division_id).
...
get(DivisionId) ->
    Sql = "SELECT division_id, status, status_label, started_at, started_by, "
          ...
          "FROM deployments WHERE division_id = ?1",
    case query_deployments_store:query(Sql, [DivisionId]) of
        {ok, [Row]} ->
            {ok, row_to_map(Row)};
        {ok, []} ->
            {error, not_found};
