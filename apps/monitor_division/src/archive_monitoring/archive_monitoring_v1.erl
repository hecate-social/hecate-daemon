...
validate(#archive_monitoring_v1{division_id = V}) when not is_binary(V); V =:= <<>> ->
    {error, {invalid_field, division_id}};
validate(_) -> ok.

to_map(#archive_monitoring_v1{division_id = V, archived_by = AB, reason = R}) ->
    #{
        <<"command_type">> => <<"archive_monitoring">>,
        <<"division_id">> => V,
        <<"archived_by">> => AB,
        <<"reason">> => R
    }.
...
get_division_id(#archive_monitoring_v1{division_id = V}) -> V.
get_archived_by(#archive_monitoring_v1{archived_by = V}) -> V.
get_reason(#archive_monitoring_v1{reason = V}) -> V.
