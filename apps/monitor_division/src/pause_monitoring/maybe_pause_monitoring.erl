...
handle(Cmd, _Context) ->
    case pause_monitoring_v1:validate(Cmd) of
        ok ->
            Event = monitoring_paused_v1:new(#{
                division_id => pause_monitoring_v1:get_division_id(Cmd),
                reason => pause_monitoring_v1:get_reason(Cmd)
            }),
            {ok, [Event]};
        {error, _} = Err -> Err
    end.
...
dispatch(Cmd) ->
    DivisionId = pause_monitoring_v1:get_division_id(Cmd),
    ...
    EvoqCmd = #evoq_command{
        ...
        payload = pause_monitoring_v1:to_map(Cmd),
