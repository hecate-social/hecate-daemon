...
validate(#archive_deployment_v1{division_id = V}) when not is_binary(V); V =:= <<>> ->
    {error, {invalid_field, division_id}};
validate(_) -> ok.

to_map(#archive_deployment_v1{division_id = V, archived_by = AB, reason = R}) ->
    #{
...
get_division_id(#archive_deployment_v1{division_id = V}) -> V.
get_archived_by(#archive_deployment_v1{archived_by = V}) -> V.
get_reason(#archive_deployment_v1{reason = V}) -> V.
