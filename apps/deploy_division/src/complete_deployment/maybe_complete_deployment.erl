...
handle(Cmd, _Context) ->
    case complete_deployment_v1:validate(Cmd) of
        ok ->
            Event = deployment_completed_v1:new(#{
                division_id => complete_deployment_v1:get_division_id(Cmd)
            }),
            {ok, [Event]};
        {error, _} = Err -> Err
    end.
...
dispatch(Cmd) ->
    DivisionId = complete_deployment_v1:get_division_id(Cmd),
