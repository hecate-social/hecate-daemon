...
start_infra() ->
    ...
    TmpDir = "/tmp/test_dispatch_deploy_" ++
...
stop_infra(#{tmp_dir := TmpDir}) ->
    reckon_db_sup:stop_store(deploy_division_store),
    os:cmd("rm -rf " ++ TmpDir),
    ok.

dispatch_persists_event() ->
    DivisionId = unique_id(<<"dep-persist">>),
    {ok, Cmd} = start_deployment_v1:new(#{division_id => DivisionId}),
    {ok, _, [_]} = maybe_start_deployment:dispatch(Cmd),
    {ok, Events} = esdb_gater_api:get_events(
        deploy_division_store, DivisionId, 0, 100, forward),
    ?assertEqual(1, length(Events)).

dispatch_returns_version_and_events() ->
    DivisionId = unique_id(<<"dep-ret">>),
    {ok, Cmd} = start_deployment_v1:new(#{division_id => DivisionId}),
    {ok, Version, EventMaps} = maybe_start_deployment:dispatch(Cmd),
...
dispatch_second_command_appends_to_stream() ->
    DivisionId = unique_id(<<"dep-stream">>),
    {ok, StartCmd} = start_deployment_v1:new(#{division_id => DivisionId}),
    {ok, _, _} = maybe_start_deployment:dispatch(StartCmd),
    {ok, ArchiveCmd} = archive_deployment_v1:new(#{division_id => DivisionId}),
    {ok, _, _} = maybe_archive_deployment:dispatch(ArchiveCmd),
    {ok, Events} = esdb_gater_api:get_events(
        deploy_division_store, DivisionId, 0, 100, forward),
    ?assertEqual(2, length(Events)).

read_back_event_data_matches() ->
    DivisionId = unique_id(<<"dep-read">>),
