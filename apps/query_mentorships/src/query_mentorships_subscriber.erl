%%% @doc Event subscriber for query_mentorships projections.
%%% Subscribes to hecate_event_store events and projects to SQLite.
-module(query_mentorships_subscriber).
...
init([]) ->
    SubIds = lists:filtermap(fun(EventType) ->
        case reckon_evoq_adapter:subscribe(
            ...
            event_type,
            EventType,
            <<"query_mentorships_subscriber_", EventType/binary>>,
            #{start_from => 0, subscriber_pid => self()}
        ) of
            {ok, SubId} ->
                io:format("[query_mentorships_subscriber] Subscribed to ~s~n", [EventType]),
                {true, SubId};
            {error, Reason} ->
    ...
    io:format("~n[query_mentorships_subscriber] Subscribed to mentor events~n"),
