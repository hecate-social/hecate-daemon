%%% @doc Emitter: channel_opened_v1 -> pg (internal pub/sub)
%%% Subscribes to dev_studio_store via evoq, broadcasts to pg group.
-module(channel_opened_v1_to_pg).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"channel_opened_v1">>).
-define(PG_GROUP, channel_opened_v1).
-define(SUB_NAME, <<"channel_opened_v1_to_pg">>).
