...
dispatch(Data) when is_map(Data) ->
    ...
    EvoqCmd = #evoq_command{
        ...
        payload = #{
            ...
            division_id  => maps:get(division_id, Data, undefined),
            agent_id     => maps:get(agent_id, Data, undefined),
            task_id      => maps:get(task_id, Data, undefined),
            model        => maps:get(model, Data, <<>>),
            tokens_in    => maps:get(tokens_in, Data, 0),
            tokens_out   => maps:get(tokens_out, Data, 0),
            cost_usd     => maps:get(cost_usd, Data, undefined)
        },
