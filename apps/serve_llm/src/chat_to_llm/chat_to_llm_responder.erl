%%% @doc Chat to LLM Responder
%%% Listens for chat requests on mesh, executes via chat_to_llm, sends response.
-module(chat_to_llm_responder).
...
init([]) ->
    Topic = hecate_topics:hope(<<"llm">>, <<"chat_to_model">>, 1),
...
handle_info({try_subscribe, Topic}, #state{subscription_ref = undefined} = State) ->
    case subscribe(Topic) of
        undefined ->
            logger:info("[chat_to_llm_responder] Mesh not connected, retrying subscription in 3s"),
            erlang:send_after(3000, self(), {try_subscribe, Topic}),
            {noreply, State};
        SubRef ->
            logger:info("[chat_to_llm_responder] Subscribed to mesh topic"),
...
handle_request(Payload) ->
    ...
    Result = chat_to_llm:chat(Model, Messages),
...
send_response(ReplyTo, RequestId, {ok, Response}) ->
    Payload = #{<<"request_id">> => RequestId, <<"status">> => <<"ok">>, <<"result">> => Response},
