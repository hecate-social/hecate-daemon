%%% @doc Projection: lifecycle events (paused/resumed/completed) -> plans table
-module(plan_lifecycle_to_sqlite_plans).
...
project_paused(Event) ->
    ...
    update_status_flags(DivisionId, [{set, ?PLAN_PAUSED}, {unset, ?PLAN_ACTIVE}]),
    query_plans_store:execute(
        "UPDATE plans SET paused_at = ?1, pause_reason = ?2 WHERE division_id = ?3",
        [PausedAt, Reason, DivisionId]).

project_resumed(Event) ->
    DivisionId = get(division_id, Event),
    update_status_flags(DivisionId, [{set, ?PLAN_ACTIVE}, {unset, ?PLAN_PAUSED}]),
    query_plans_store:execute(
        "UPDATE plans SET paused_at = NULL, pause_reason = NULL WHERE division_id = ?1",
        [DivisionId]).

project_completed(Event) ->
    DivisionId = get(division_id, Event),
    CompletedAt = get(completed_at, Event),
    update_status_flags(DivisionId, [{set, ?PLAN_COMPLETED}, {unset, ?PLAN_ACTIVE}]),
    query_plans_store:execute(
        "UPDATE plans SET completed_at = ?1 WHERE division_id = ?2",
...
update_status_flags(DivisionId, Ops) ->
    case query_plans_store:query(
        ...
        {ok, [{CurrentStatus}]} ->
            ...
            query_plans_store:execute(
                "UPDATE plans SET status = ?1, status_label = ?2 WHERE division_id = ?3",
