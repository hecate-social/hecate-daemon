%%% @doc API handler: POST /api/cartwheels/:cartwheel_id/discovery/findings/record
%%%
%%% Records a finding during the discovery phase.
%%% @end
-module(record_finding_api).
...
do_record(Params, Req) ->
    ...
    CmdParams = #{
        ...
        category => hecate_api_utils:get_field(category, Params, <<"requirement">>),
        content => hecate_api_utils:get_field(content, Params),
        priority => hecate_api_utils:get_field(priority, Params, <<"should">>)
    },
    dispatch(record_finding_v1, maybe_record_finding, CmdParams, Req).
