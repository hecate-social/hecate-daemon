%%% @doc API handler: POST /api/cartwheels/:cartwheel_id/deployment/complete
%%%
%%% Completes the deployment phase for a cartwheel.
%%% @end
-module(complete_deployment_api).
...
dispatch(CmdParams, Req) ->
    case complete_deployment_v1:new(CmdParams) of
        {ok, Cmd} -> do_dispatch(Cmd, Req);
        {error, Reason} -> hecate_api_utils:bad_request(Reason, Req)
    end.

do_dispatch(Cmd, Req) ->
    case maybe_complete_deployment:dispatch(Cmd) of
        {ok, Version, Events} ->
