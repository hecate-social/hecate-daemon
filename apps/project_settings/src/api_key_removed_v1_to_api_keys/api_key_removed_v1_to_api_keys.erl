%%% @doc Projection: api_key_removed_v1 -> api_keys table
-module(api_key_removed_v1_to_api_keys).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        hecate_event_store, event_type, <<"api_key_removed_v1">>,
        <<"prj_api_key_removed">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
project(Data) ->
    ...
    Sql = "DELETE FROM api_keys WHERE provider = ?1",
    project_settings_store:execute(Sql, [Provider]).
