-module(unsubscribe_topic_api).
-export([init/2, routes/0]).

routes() -> [{"/api/node/subscriptions/unsubscribe", ?MODULE, []}].
...
handle_post(Req0, _State) ->
    case hecate_api_utils:read_json_body(Req0) of
        {ok, #{<<"agent_identity">> := Agent, <<"topic">> := Topic}, Req1} ->
            ...
            Cmd = unsubscribe_topic_v1:new(Agent, Topic, Timestamp),
            dispatch_result(maybe_unsubscribe_topic:dispatch(Cmd), Req1);
        {ok, _, Req1} ->
