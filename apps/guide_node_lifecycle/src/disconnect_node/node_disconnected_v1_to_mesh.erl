%%% @doc Mesh emitter: node_disconnected_v1 → Macula Mesh
-module(node_disconnected_v1_to_mesh).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        hecate_event_store, event_type, <<"node_disconnected_v1">>,
        <<"mesh_node_disconnected">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.mesh.disconnected">>, EventData),
    {noreply, State};
