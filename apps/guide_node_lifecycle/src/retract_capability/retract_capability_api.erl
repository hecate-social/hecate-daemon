-module(retract_capability_api).
-export([init/2, routes/0]).

routes() -> [{"/api/node/capabilities/:mri/retract", ?MODULE, []}].
    ...
    case hecate_api_utils:read_json_body(Req0) of
        {ok, #{<<"agent_identity">> := Agent} = Payload, Req1} ->
            ...
            case retract_capability_v1:new(MRI, Agent, Reason) of
                {ok, Cmd} ->
                    dispatch_result(maybe_retract_capability:dispatch(Cmd), MRI, Req1);
                {error, ErrReason} ->
                    hecate_api_utils:json_error(400, ErrReason, Req1)
            end;
        {ok, _, Req1} ->
