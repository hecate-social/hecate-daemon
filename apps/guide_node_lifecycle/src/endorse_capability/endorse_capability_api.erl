-module(endorse_capability_api).
-export([init/2, routes/0]).

routes() -> [{"/api/node/mesh/endorse", ?MODULE, []}].
...
handle_post(Req0, _State) ->
    case hecate_api_utils:read_json_body(Req0) of
        {ok, #{<<"endorser_identity">> := Endorser, <<"capability_mri">> := Mri} = P, Req1} ->
            Comment = maps:get(<<"comment">>, P, undefined),
            Cmd = endorse_capability_v1:new(Endorser, Mri, Comment),
            dispatch_result(maybe_endorse_capability:dispatch(Cmd), Req1);
        {ok, _, Req1} ->
            hecate_api_utils:bad_request(<<"Missing endorser_identity or capability_mri">>, Req1);
