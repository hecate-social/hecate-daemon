%%% @doc Mesh emitter: node_connected_v1 → Macula Mesh
-module(node_connected_v1_to_mesh).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        hecate_event_store, event_type, <<"node_connected_v1">>,
        <<"mesh_node_connected">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.mesh.connected">>, EventData),
    {noreply, State};
