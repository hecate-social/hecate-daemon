...
%%% OCI image existence), then dispatches initiate_license_v1 to
%%% create a DRAFT catalog entry.
%%%
%%% Replaces the old install_plugin_from_url endpoint which bypassed
%%% the license/catalog domain entirely.
%%% @end
    ...
    case cowboy_req:header(<<"x-hecate-user-id">>, Req0) of
        ...
        SellerId ->
            case hecate_api_utils:read_json_body(Req0) of
                {ok, Params, Req1} ->
                    do_publish_from_url(Params, SellerId, Req1);
                {error, invalid_json, Req1} ->
                    hecate_api_utils:bad_request(<<"Invalid JSON">>, Req1)
            end
    end.

do_publish_from_url(Params, SellerId, Req) ->
...
fetch_and_publish(ManifestUrl, SellerId, Req) ->
    case hackney:get(ManifestUrl, [{<<"Accept">>, <<"application/json">>}], <<>>, [with_body]) of
        {ok, 200, _Headers, Body} ->
            parse_and_publish(Body, ManifestUrl, SellerId, Req);
...
parse_and_publish(Body, ManifestUrl, SellerId, Req) ->
    try json:decode(Body) of
        Manifest when is_map(Manifest) ->
            validate_and_publish(Manifest, Body, ManifestUrl, SellerId, Req);
        _ ->
...
validate_and_publish(Manifest, RawBody, ManifestUrl, SellerId, Req) ->
...
compute_trust(RawBody, OciImage, Version) ->
    ...
    %% 2. Sign checksum with seller's Ed25519 key
    {SignatureB64, SellerMRI} = sign_checksum(ChecksumBin),
    ...
    #{
        ...
        seller_signature => SignatureB64,
        seller_mri => SellerMRI,
...
dispatch_initiate(Name, Version, Org, OciImage, Manifest,
    ...
    CmdParams = #{
        ...
        publisher_identity => maps:get(seller_mri, TrustData, undefined),
