%%% @doc Listener for container_pull_started_v1 events.
%%% Subscribes to evoq, projects to SQLite plugins table.
-module(on_container_pull_started_v1_to_sqlite_plugins).
...
-define(SUB_NAME, <<"container_pull_started_v1_to_sqlite_plugins">>).
...
handle_info({events, Events}, State) ->
    lists:foreach(fun(#evoq_event{data = Data}) ->
        case container_pull_started_v1_to_sqlite_plugins:project(Data) of
            ok -> ok;
            {error, Reason} ->
