%%% @doc Listener for plugin_upgraded_v1 events.
%%% Subscribes to evoq, projects to SQLite plugins table.
-module(on_plugin_upgraded_v1_to_sqlite_plugins).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"plugin_upgraded_v1">>).
-define(SUB_NAME, <<"plugin_upgraded_v1_to_sqlite_plugins">>).
...
handle_info({events, Events}, State) ->
    lists:foreach(fun(E) ->
        case plugin_upgraded_v1_to_sqlite_plugins:project(projection_event:to_map(E)) of
            ok -> ok;
            {error, Reason} ->
