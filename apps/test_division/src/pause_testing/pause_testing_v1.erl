...
validate(#pause_testing_v1{division_id = V}) when not is_binary(V); V =:= <<>> ->
    {error, {invalid_field, division_id}};
validate(_) -> ok.

to_map(#pause_testing_v1{division_id = V, reason = R}) ->
    #{
...
get_division_id(#pause_testing_v1{division_id = V}) -> V.
get_reason(#pause_testing_v1{reason = V}) -> V.
