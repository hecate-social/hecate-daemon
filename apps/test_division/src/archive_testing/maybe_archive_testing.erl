...
handle(Cmd, _Context) ->
    case archive_testing_v1:validate(Cmd) of
        ok ->
            Event = testing_archived_v1:new(#{
                division_id => archive_testing_v1:get_division_id(Cmd),
                archived_by => archive_testing_v1:get_archived_by(Cmd),
                reason => archive_testing_v1:get_reason(Cmd)
            }),
            {ok, [Event]};
        {error, _} = Err -> Err
    end.
...
dispatch(Cmd) ->
    DivisionId = archive_testing_v1:get_division_id(Cmd),
    ...
    EvoqCmd = #evoq_command{
        ...
        payload = archive_testing_v1:to_map(Cmd),
