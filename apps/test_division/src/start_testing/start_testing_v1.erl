...
validate(#start_testing_v1{division_id = V}) when not is_binary(V); V =:= <<>> ->
    {error, {invalid_field, division_id}};
validate(_) -> ok.

to_map(#start_testing_v1{division_id = V, started_by = SB}) ->
    #{
...
get_division_id(#start_testing_v1{division_id = V}) -> V.
