...
start_infra() ->
    ...
    TmpDir = "/tmp/test_dispatch_rescue_" ++
...
stop_infra(#{tmp_dir := TmpDir}) ->
    reckon_db_sup:stop_store(rescue_division_store),
    os:cmd("rm -rf " ++ TmpDir),
    ok.
...
dispatch_persists_event() ->
    ...
    {ok, Events} = esdb_gater_api:get_events(
        rescue_division_store, DivisionId, 0, 100, forward),
    ?assertEqual(1, length(Events)).

dispatch_returns_version_and_events() ->
    DivisionId = unique_id(<<"res-ret">>),
    {ok, Cmd} = start_rescue_v1:new(#{
        division_id => DivisionId,
        incident_id => <<"inc-2">>
    }),
    {ok, Version, EventMaps} = maybe_start_rescue:dispatch(Cmd),
...
dispatch_second_command_appends_to_stream() ->
    DivisionId = unique_id(<<"res-stream">>),
    {ok, StartCmd} = start_rescue_v1:new(#{
        division_id => DivisionId,
        incident_id => <<"inc-3">>
    }),
    {ok, _, _} = maybe_start_rescue:dispatch(StartCmd),
    {ok, ArchiveCmd} = archive_rescue_v1:new(#{division_id => DivisionId}),
    {ok, _, _} = maybe_archive_rescue:dispatch(ArchiveCmd),
    {ok, Events} = esdb_gater_api:get_events(
        rescue_division_store, DivisionId, 0, 100, forward),
    ?assertEqual(2, length(Events)).

read_back_event_data_matches() ->
    DivisionId = unique_id(<<"res-read">>),
    {ok, Cmd} = start_rescue_v1:new(#{
        division_id => DivisionId,
        incident_id => <<"inc-4">>,
