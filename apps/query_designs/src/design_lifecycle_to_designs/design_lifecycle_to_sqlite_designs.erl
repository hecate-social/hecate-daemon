%%% @doc Projection: lifecycle events (paused/resumed/completed) -> designs table
-module(design_lifecycle_to_sqlite_designs).
...
project_paused(Event) ->
    ...
    update_status_flags(DivisionId, [{set, ?DESIGN_PAUSED}, {unset, ?DESIGN_ACTIVE}]),
    query_designs_store:execute(
        "UPDATE designs SET paused_at = ?1, pause_reason = ?2 WHERE division_id = ?3",
        [PausedAt, Reason, DivisionId]).

project_resumed(Event) ->
    DivisionId = get(division_id, Event),
    update_status_flags(DivisionId, [{set, ?DESIGN_ACTIVE}, {unset, ?DESIGN_PAUSED}]),
    query_designs_store:execute(
        "UPDATE designs SET paused_at = NULL, pause_reason = NULL WHERE division_id = ?1",
        [DivisionId]).

project_completed(Event) ->
    DivisionId = get(division_id, Event),
    CompletedAt = get(completed_at, Event),
    update_status_flags(DivisionId, [{set, ?DESIGN_COMPLETED}, {unset, ?DESIGN_ACTIVE}]),
    query_designs_store:execute(
        "UPDATE designs SET completed_at = ?1 WHERE division_id = ?2",
...
update_status_flags(DivisionId, Ops) ->
    case query_designs_store:query(
        ...
        {ok, [{CurrentStatus}]} ->
            ...
            query_designs_store:execute(
                "UPDATE designs SET status = ?1, status_label = ?2 WHERE division_id = ?3",
