%%% @doc Projection: license_retracted_v1 -> catalog ETS read model.
%%% Resets a catalog entry to retracted state.
-module(license_retracted_v1_to_catalog).
-behaviour(evoq_projection).
-export([interested_in/0, init/1, project/4]).

-define(TABLE, catalog).

interested_in() -> [<<"license_retracted_v1">>].
