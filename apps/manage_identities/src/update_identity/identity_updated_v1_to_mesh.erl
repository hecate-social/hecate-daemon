%%% @doc Mesh projection: identity_updated_v1 → Macula Mesh
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_identities_store, event_type, <<"identity_updated_v1">>,
        <<"mesh_identity_updated">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.identity.updated">>, EventData),
    {noreply, State};
