%%% @doc Mesh projection: subscribed_v1 → Macula Mesh
-module(subscribed_v1_to_mesh).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_subscriptions_store, event_type, <<"subscribed_v1">>,
        <<"mesh_subscribed">>, #{start_from => 0, subscriber_pid => self()}
    ),
