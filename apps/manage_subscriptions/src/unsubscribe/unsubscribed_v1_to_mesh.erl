%%% @doc Mesh projection: unsubscribed_v1 → Macula Mesh
-module(unsubscribed_v1_to_mesh).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_subscriptions_store, event_type, <<"unsubscribed_v1">>,
        <<"mesh_unsubscribed">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.subscription.unsubscribed">>, EventData),
    {noreply, State};
...
terminate(_Reason, #state{subscription_id = SubId}) ->
    case SubId of
        undefined -> ok;
        _ -> reckon_evoq_adapter:unsubscribe(manage_subscriptions_store, SubId)
    end.
