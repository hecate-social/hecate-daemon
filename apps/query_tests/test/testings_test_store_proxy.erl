-module(testings_test_store_proxy).
-behaviour(gen_server).
-export([start/1, stop/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).
-record(state, {db :: reference(), path :: string()}).
start(DbPath) -> gen_server:start({local, query_tests_store}, ?MODULE, DbPath, []).
stop() -> gen_server:stop(query_tests_store).
init(DbPath) ->
...
create_tables(Db) ->
    lists:foreach(fun(Sql) -> ok = esqlite3:exec(Db, Sql) end, [
        "CREATE TABLE IF NOT EXISTS testings (division_id TEXT PRIMARY KEY, status INTEGER NOT NULL DEFAULT 1, status_label TEXT DEFAULT 'Initiated', started_at INTEGER, started_by TEXT, paused_at INTEGER, pause_reason TEXT, completed_at INTEGER);",
        "CREATE TABLE IF NOT EXISTS test_suites (suite_id TEXT PRIMARY KEY, division_id TEXT NOT NULL, suite_name TEXT NOT NULL, suite_type TEXT, target_module TEXT, run_at INTEGER NOT NULL);",
        "CREATE TABLE IF NOT EXISTS test_results (result_id TEXT PRIMARY KEY, division_id TEXT NOT NULL, suite_id TEXT NOT NULL, test_name TEXT NOT NULL, status TEXT, details TEXT, recorded_at INTEGER NOT NULL);",
