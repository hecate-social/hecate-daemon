%%% @doc Mesh projection: capability_revoked_v1 → Macula Mesh
-module(capability_revoked_v1_to_mesh).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_ucan_store, event_type, <<"capability_revoked_v1">>,
        <<"mesh_capability_revoked">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.ucan.revoked">>, EventData),
    {noreply, State};
...
terminate(_Reason, #state{subscription_id = SubId}) ->
    case SubId of
        undefined -> ok;
        _ -> reckon_evoq_adapter:unsubscribe(manage_ucan_store, SubId)
    end.
