-module(handle_get_disputes_page).
...
%% @doc Handle get_disputes_page query - list disputes with optional filtering.
-spec handle(get_disputes_page_v1:get_disputes_page_v1()) -> {ok, [map()]} | {error, term()}.
handle(Query) ->
    #{
        agent_identity := Identity,
        status_filter := StatusFilter
    } = get_disputes_page_v1:to_map(Query),
