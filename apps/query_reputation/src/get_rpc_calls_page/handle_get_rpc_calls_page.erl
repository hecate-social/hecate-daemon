-module(handle_get_rpc_calls_page).
...
%% @doc Handle get_rpc_calls_page query - list RPC calls for an agent with optional filtering.
-spec handle(get_rpc_calls_page_v1:get_rpc_calls_page_v1()) -> {ok, [map()]} | {error, term()}.
handle(Query) ->
    #{
        agent_identity := Identity,
        limit := Limit,
        success_filter := Filter
    } = get_rpc_calls_page_v1:to_map(Query),
