...
handle(Cmd, _Context) ->
    case pause_plan_v1:validate(Cmd) of
        ok ->
            Event = plan_paused_v1:new(#{
                division_id => pause_plan_v1:get_division_id(Cmd),
                reason => pause_plan_v1:get_reason(Cmd)
            }),
            {ok, [Event]};
        {error, _} = Err -> Err
    end.
...
dispatch(Cmd) ->
    DivisionId = pause_plan_v1:get_division_id(Cmd),
    Timestamp = erlang:system_time(millisecond),
    EvoqCmd = #evoq_command{
        command_id = generate_command_id(DivisionId, Timestamp),
        command_type = pause_plan,
        aggregate_type = plan_aggregate,
        aggregate_id = DivisionId,
        payload = pause_plan_v1:to_map(Cmd),
