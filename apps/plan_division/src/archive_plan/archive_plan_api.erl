...
handle_post(Req0, _State) ->
    ...
    case archive_plan_v1:new(#{division_id => DivisionId, archived_by => ArchivedBy, reason => Reason}) of
        {ok, Cmd} ->
            case maybe_archive_plan:dispatch(Cmd) of
                {ok, Version, Events} ->
                    lists:foreach(fun(E) ->
                        plan_archived_v1_to_pg:emit(E)
                    end, Events),
                    RespBody = #{
