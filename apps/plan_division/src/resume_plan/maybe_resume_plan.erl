...
handle(Cmd, _Context) ->
    case resume_plan_v1:validate(Cmd) of
        ok ->
            Event = plan_resumed_v1:new(#{
                division_id => resume_plan_v1:get_division_id(Cmd)
            }),
            {ok, [Event]};
        {error, _} = Err -> Err
    end.
...
dispatch(Cmd) ->
    DivisionId = resume_plan_v1:get_division_id(Cmd),
    ...
    EvoqCmd = #evoq_command{
        ...
        payload = resume_plan_v1:to_map(Cmd),
