-module(endorse_learning_api).
-export([init/2, routes/0]).

routes() -> [{"/api/mentors/learnings/:learning_id/endorse", ?MODULE, []}].
...
handle_post(Req0, _State) ->
    ...
    case endorse_learning_v1:new(CmdParams) of
        {ok, Cmd} ->
            dispatch_result(maybe_endorse_learning:dispatch(Cmd), Req0);
        {error, Reason} ->
