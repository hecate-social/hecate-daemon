%%% @doc Listener: torch_initiated_v1 from pg -> sqlite torches projection
%%% Subscribes to torch_initiated_v1 pg group and triggers projection.
...
handle_info({torch_initiated_v1, Event}, State) ->
    logger:info("[~s] Received torch_initiated_v1 event", [?MODULE]),
    %% Call the projection
    Result = torch_initiated_v1_to_sqlite_torches:project(Event),
    case Result of
        ok ->
