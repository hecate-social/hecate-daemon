%%% @doc Mesh projection: agent_followed_v1 → Macula Mesh
-module(agent_followed_v1_to_mesh).
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_social_store, event_type, <<"agent_followed_v1">>,
        <<"mesh_agent_followed">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.social.followed">>, EventData),
    {noreply, State};
