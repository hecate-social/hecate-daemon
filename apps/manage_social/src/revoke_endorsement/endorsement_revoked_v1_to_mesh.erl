%%% @doc Mesh projection: endorsement_revoked_v1 → Macula Mesh
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_social_store, event_type, <<"endorsement_revoked_v1">>,
        <<"mesh_endorsement_revoked">>, #{start_from => 0, subscriber_pid => self()}
    ),
...
handle_info({event, #evoq_event{data = EventData}}, State) ->
    hecate_mesh_client:publish(<<"hecate.social.endorsement_revoked">>, EventData),
    {noreply, State};
