%%% @doc Mesh projection: capability_endorsed_v1 → Macula Mesh
...
init([]) ->
    {ok, SubId} = reckon_evoq_adapter:subscribe(
        manage_social_store, event_type, <<"capability_endorsed_v1">>,
        <<"mesh_capability_endorsed">>, #{start_from => 0, subscriber_pid => self()}
    ),
