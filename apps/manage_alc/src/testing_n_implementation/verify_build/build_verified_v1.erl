...
-export([get_project_id/1, get_build_id/1, get_result/1, get_notes/1,
         get_verified_at/1]).
...
new(#{project_id := ProjectId, build_id := BuildId, result := Result} = Params) ->
    #build_verified_v1{
...
to_map(#build_verified_v1{} = E) ->
    #{
        ...
        project_id => E#build_verified_v1.project_id,
...
from_map(#{project_id := ProjectId, build_id := BuildId, result := Result} = Map) ->
...
%% Accessors
get_project_id(#build_verified_v1{project_id = V}) -> V.
