...
handle(Cmd) ->
    ProjectId = implement_spoke_v1:get_project_id(Cmd),
    SpokeId = implement_spoke_v1:get_spoke_id(Cmd),
    case validate_command(ProjectId, SpokeId) of
        ok ->
...
validate_command(ProjectId, SpokeId) when
...
create_event(Cmd) ->
    spoke_implemented_v1:new(#{
        project_id => implement_spoke_v1:get_project_id(Cmd),
