...
-export([get_project_id/1, get_plan_id/1, get_title/1,
...
new(#{project_id := ProjectId, plan_id := PlanId, title := Title} = Params) ->
    #plan_drafted_v1{
...
to_map(#plan_drafted_v1{} = E) ->
    #{
        ...
        project_id => E#plan_drafted_v1.project_id,
...
from_map(#{project_id := ProjectId, plan_id := PlanId, title := Title} = Map) ->
    {ok, #plan_drafted_v1{
        project_id = ProjectId,
        plan_id = PlanId,
        title = Title,
...
%% Accessors
get_project_id(#plan_drafted_v1{project_id = V}) -> V.
