...
handle(Cmd) ->
    ProjectId = transition_phase_v1:get_project_id(Cmd),
    FromPhase = transition_phase_v1:get_from_phase(Cmd),
    ToPhase = transition_phase_v1:get_to_phase(Cmd),
    case validate_command(ProjectId, FromPhase, ToPhase) of
        ok ->
...
validate_command(ProjectId, FromPhase, ToPhase) when
...
create_event(Cmd) ->
    phase_transitioned_v1:new(#{
        project_id => transition_phase_v1:get_project_id(Cmd),
