%%% @doc ALC aggregate
%%% Maintains ALC project state and applies events.
%%% Handles orchestration events (initiate_project, transition_phase, revisit_phase)
%%% and DnA events (discovery_started, finding_recorded, term_defined, discovery_completed).
-module(alc_aggregate).
...
execute(#{command_type := <<"initiate_project">>} = Payload, State) ->
    execute_initiate_project(Payload, State);
...
execute_initiate_project(Payload, #alc_state{status = 0}) ->
    {ok, Cmd} = initiate_project_v1:from_map(Payload),
    convert_events(maybe_initiate_project:handle(Cmd), fun project_initiated_v1:to_map/1);
execute_initiate_project(_Payload, _State) ->
    {error, project_already_initiated}.

execute_start_discovery(Payload, #alc_state{current_phase = <<"discovery_n_analysis">>}) ->
...
execute_start_discovery(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_start_discovery(_Payload, _State) ->
    {error, not_in_discovery_phase}.

execute_record_finding(Payload, #alc_state{current_phase = <<"discovery_n_analysis">>}) ->
    {ok, Cmd} = record_finding_v1:from_map(Payload),
    convert_events(maybe_record_finding:handle(Cmd), fun finding_recorded_v1:to_map/1);
execute_record_finding(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_record_finding(_Payload, _State) ->
    {error, not_in_discovery_phase}.

execute_define_term(Payload, #alc_state{current_phase = <<"discovery_n_analysis">>}) ->
    {ok, Cmd} = define_term_v1:from_map(Payload),
    convert_events(maybe_define_term:handle(Cmd), fun term_defined_v1:to_map/1);
execute_define_term(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_define_term(_Payload, _State) ->
    {error, not_in_discovery_phase}.

execute_complete_discovery(Payload, #alc_state{current_phase = <<"discovery_n_analysis">>, finding_count = FC, term_count = TC})
  when FC > 0, TC > 0 ->
...
execute_complete_discovery(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_complete_discovery(_Payload, #alc_state{current_phase = Phase}) when Phase =/= <<"discovery_n_analysis">> ->
    {error, not_in_discovery_phase};
execute_complete_discovery(_Payload, #alc_state{finding_count = 0}) ->
    {error, no_findings_recorded};
execute_complete_discovery(_Payload, #alc_state{term_count = 0}) ->
    {error, no_terms_defined}.

execute_transition_phase(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_transition_phase(Payload, #alc_state{current_phase = CurrentPhase} = State) ->
...
check_gate(#alc_state{current_phase = <<"discovery_n_analysis">>, finding_count = FC, term_count = TC})
  when FC > 0, TC > 0 ->
    ok;
check_gate(#alc_state{current_phase = <<"discovery_n_analysis">>}) ->
    {error, gate_conditions_not_met};
check_gate(#alc_state{current_phase = <<"architecture_n_planning">>, dossier_count = DC, spoke_count = SC, plan_approved = PA})
  when DC > 0, SC > 0, PA =:= true ->
    ok;
check_gate(#alc_state{current_phase = <<"architecture_n_planning">>}) ->
    {error, gate_conditions_not_met};
check_gate(#alc_state{current_phase = <<"testing_n_implementation">>, skeleton_created = SC, implemented_spoke_count = ISC, build_verified = BV})
  when SC =:= true, ISC > 0, BV =:= true ->
    ok;
check_gate(#alc_state{current_phase = <<"testing_n_implementation">>}) ->
    {error, gate_conditions_not_met};
check_gate(#alc_state{current_phase = <<"deployment_n_operations">>, deployment_count = DC, active_incidents = AI})
  when DC > 0, AI =:= 0 ->
    ok;
check_gate(#alc_state{current_phase = <<"deployment_n_operations">>}) ->
    {error, gate_conditions_not_met};
check_gate(_State) ->
    ok.

execute_start_architecture(Payload, #alc_state{current_phase = <<"architecture_n_planning">>}) ->
...
execute_start_architecture(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_start_architecture(_Payload, _State) ->
    {error, not_in_architecture_phase}.

execute_define_dossier(Payload, #alc_state{current_phase = <<"architecture_n_planning">>}) ->
    {ok, Cmd} = define_dossier_v1:from_map(Payload),
    convert_events(maybe_define_dossier:handle(Cmd), fun dossier_defined_v1:to_map/1);
execute_define_dossier(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_define_dossier(_Payload, _State) ->
    {error, not_in_architecture_phase}.

execute_inventory_spoke(Payload, #alc_state{current_phase = <<"architecture_n_planning">>}) ->
...
execute_inventory_spoke(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_inventory_spoke(_Payload, _State) ->
    {error, not_in_architecture_phase}.

execute_draft_plan(Payload, #alc_state{current_phase = <<"architecture_n_planning">>}) ->
    {ok, Cmd} = draft_plan_v1:from_map(Payload),
    convert_events(maybe_draft_plan:handle(Cmd), fun plan_drafted_v1:to_map/1);
execute_draft_plan(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_draft_plan(_Payload, _State) ->
    {error, not_in_architecture_phase}.

execute_approve_plan(Payload, #alc_state{current_phase = <<"architecture_n_planning">>}) ->
    {ok, Cmd} = approve_plan_v1:from_map(Payload),
    convert_events(maybe_approve_plan:handle(Cmd), fun plan_approved_v1:to_map/1);
execute_approve_plan(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_approve_plan(_Payload, _State) ->
    {error, not_in_architecture_phase}.

execute_complete_architecture(Payload, #alc_state{current_phase = <<"architecture_n_planning">>, dossier_count = DC, spoke_count = SC, plan_approved = PA})
  when DC > 0, SC > 0, PA =:= true ->
...
execute_complete_architecture(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_complete_architecture(_Payload, #alc_state{current_phase = Phase}) when Phase =/= <<"architecture_n_planning">> ->
    {error, not_in_architecture_phase};
execute_complete_architecture(_Payload, #alc_state{dossier_count = 0}) ->
    {error, no_dossiers_defined};
execute_complete_architecture(_Payload, #alc_state{spoke_count = 0}) ->
    {error, no_spokes_inventoried};
execute_complete_architecture(_Payload, #alc_state{plan_approved = false}) ->
    {error, plan_not_approved}.

execute_start_testing(Payload, #alc_state{current_phase = <<"testing_n_implementation">>}) ->
    {ok, Cmd} = start_testing_v1:from_map(Payload),
    convert_events(maybe_start_testing:handle(Cmd), fun testing_started_v1:to_map/1);
execute_start_testing(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_start_testing(_Payload, _State) ->
    {error, not_in_testing_phase}.

execute_create_skeleton(Payload, #alc_state{current_phase = <<"testing_n_implementation">>}) ->
...
execute_create_skeleton(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_create_skeleton(_Payload, _State) ->
    {error, not_in_testing_phase}.

execute_implement_spoke(Payload, #alc_state{current_phase = <<"testing_n_implementation">>}) ->
...
execute_implement_spoke(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_implement_spoke(_Payload, _State) ->
    {error, not_in_testing_phase}.

execute_verify_build(Payload, #alc_state{current_phase = <<"testing_n_implementation">>}) ->
    {ok, Cmd} = verify_build_v1:from_map(Payload),
    convert_events(maybe_verify_build:handle(Cmd), fun build_verified_v1:to_map/1);
execute_verify_build(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_verify_build(_Payload, _State) ->
    {error, not_in_testing_phase}.

execute_complete_testing(Payload, #alc_state{current_phase = <<"testing_n_implementation">>, skeleton_created = SC, implemented_spoke_count = ISC, build_verified = BV})
  when SC =:= true, ISC > 0, BV =:= true ->
...
execute_complete_testing(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_complete_testing(_Payload, #alc_state{current_phase = Phase}) when Phase =/= <<"testing_n_implementation">> ->
    {error, not_in_testing_phase};
execute_complete_testing(_Payload, #alc_state{skeleton_created = false}) ->
    {error, skeleton_not_created};
execute_complete_testing(_Payload, #alc_state{implemented_spoke_count = 0}) ->
    {error, no_spokes_implemented};
execute_complete_testing(_Payload, #alc_state{build_verified = false}) ->
    {error, build_not_verified}.

execute_start_deployment(Payload, #alc_state{current_phase = <<"deployment_n_operations">>}) ->
...
execute_start_deployment(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_start_deployment(_Payload, _State) ->
    {error, not_in_deployment_phase}.

execute_record_deployment(Payload, #alc_state{current_phase = <<"deployment_n_operations">>}) ->
...
execute_record_deployment(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_record_deployment(_Payload, _State) ->
    {error, not_in_deployment_phase}.

execute_report_incident(Payload, #alc_state{current_phase = <<"deployment_n_operations">>}) ->
...
execute_report_incident(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_report_incident(_Payload, _State) ->
    {error, not_in_deployment_phase}.

execute_resolve_incident(Payload, #alc_state{current_phase = <<"deployment_n_operations">>}) ->
...
execute_resolve_incident(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_resolve_incident(_Payload, _State) ->
    {error, not_in_deployment_phase}.

execute_complete_deployment(Payload, #alc_state{current_phase = <<"deployment_n_operations">>, deployment_count = DC, active_incidents = AI})
  when DC > 0, AI =:= 0 ->
...
execute_complete_deployment(_Payload, #alc_state{project_id = undefined}) ->
    {error, project_not_found};
execute_complete_deployment(_Payload, #alc_state{current_phase = Phase}) when Phase =/= <<"deployment_n_operations">> ->
    {error, not_in_deployment_phase};
execute_complete_deployment(_Payload, #alc_state{deployment_count = 0}) ->
    {error, no_deployments_recorded};
execute_complete_deployment(_Payload, #alc_state{active_incidents = AI}) when AI > 0 ->
...
apply_event(#{event_type := <<"project_initiated_v1">>} = E, State) ->
    State#alc_state{
        project_id = maps:get(project_id, E),
        project_name = maps:get(name, E),
...
apply_event(#{event_type := <<"finding_recorded_v1">>} = _E, State) ->
    State#alc_state{
        finding_count = State#alc_state.finding_count + 1
    };
apply_event(#{event_type := <<"term_defined_v1">>} = _E, State) ->
    State#alc_state{
        term_count = State#alc_state.term_count + 1
    };
apply_event(#{event_type := <<"discovery_completed_v1">>} = _E, State) ->
    State#alc_state{
        status = State#alc_state.status bor ?DISCOVERY_COMPLETE
    };
apply_event(#{event_type := <<"phase_transitioned_v1">>} = E, State) ->
    ...
    State#alc_state{
        ...
        status = State#alc_state.status bor StatusFlag,
...
apply_event(#{event_type := <<"dossier_defined_v1">>} = _E, State) ->
    State#alc_state{
        dossier_count = State#alc_state.dossier_count + 1
    };
apply_event(#{event_type := <<"spoke_inventoried_v1">>} = _E, State) ->
    State#alc_state{
        spoke_count = State#alc_state.spoke_count + 1
    };
...
apply_event(#{event_type := <<"architecture_completed_v1">>} = _E, State) ->
    State#alc_state{
        status = State#alc_state.status bor ?ARCHITECTURE_COMPLETE
    };
...
apply_event(#{event_type := <<"spoke_implemented_v1">>} = _E, State) ->
    State#alc_state{
        implemented_spoke_count = State#alc_state.implemented_spoke_count + 1
    };
...
apply_event(#{event_type := <<"testing_completed_v1">>} = _E, State) ->
    State#alc_state{
        status = State#alc_state.status bor ?TESTING_COMPLETE
    };
...
apply_event(#{event_type := <<"deployment_recorded_v1">>} = _E, State) ->
    State#alc_state{
        deployment_count = State#alc_state.deployment_count + 1
    };
apply_event(#{event_type := <<"incident_reported_v1">>} = _E, State) ->
    State#alc_state{
        active_incidents = State#alc_state.active_incidents + 1
    };
apply_event(#{event_type := <<"incident_resolved_v1">>} = _E, State) ->
    State#alc_state{
        active_incidents = max(0, State#alc_state.active_incidents - 1)
    };
apply_event(#{event_type := <<"deployment_completed_v1">>} = _E, State) ->
    State#alc_state{
        status = State#alc_state.status bor 256,
