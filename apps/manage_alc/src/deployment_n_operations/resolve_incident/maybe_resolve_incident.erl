...
handle(Cmd) ->
    ProjectId = resolve_incident_v1:get_project_id(Cmd),
    IncidentId = resolve_incident_v1:get_incident_id(Cmd),
    Resolution = resolve_incident_v1:get_resolution(Cmd),
    case validate_command(ProjectId, IncidentId, Resolution) of
        ok ->
...
validate_command(ProjectId, IncidentId, Resolution) when
...
create_event(Cmd) ->
    incident_resolved_v1:new(#{
        project_id => resolve_incident_v1:get_project_id(Cmd),
