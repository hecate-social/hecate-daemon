...
%%% Requests termination of a running plugin.
%%% Requires plugin_id in POST body.
-module(request_plugin_termination_api).
...
do_stop(PluginId, Req) ->
    case request_plugin_termination_v1:new(#{plugin_id => PluginId}) of
        {ok, Cmd} -> dispatch(Cmd, Req);
        {error, Reason} -> hecate_api_utils:bad_request(Reason, Req)
    end.

dispatch(Cmd, Req) ->
    case maybe_request_plugin_termination:dispatch(Cmd) of
        {ok, Version, _EventMaps} ->
            hecate_api_utils:json_ok(200, #{
                plugin_id => request_plugin_termination_v1:get_plugin_id(Cmd),
