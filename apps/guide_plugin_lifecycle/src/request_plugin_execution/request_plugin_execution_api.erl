%%% @doc API handler: POST /api/plugins/start
%%% Requests execution of an installed plugin.
%%% Requires plugin_id in POST body.
-module(request_plugin_execution_api).

-export([init/2, routes/0]).

routes() -> [{"/api/plugins/start", ?MODULE, []}].
    ...
    case hecate_api_utils:read_json_body(Req0) of
        {ok, Body, Req1} ->
            case hecate_api_utils:get_field(plugin_id, Body) of
                ...
                PluginId ->
                    do_start(PluginId, Req1)
            end;
        {error, invalid_json, Req1} ->
            hecate_api_utils:bad_request(<<"Invalid JSON">>, Req1)
    end.

do_start(PluginId, Req) ->
    ...
    case maybe_request_plugin_execution:dispatch(Cmd) of
        {ok, Version, _EventMaps} ->
            hecate_api_utils:json_ok(200, #{
                plugin_id => request_plugin_execution_v1:get_plugin_id(Cmd),
                status => <<"requested">>,
                version => Version
            }, Req);
        {error, Reason} ->
