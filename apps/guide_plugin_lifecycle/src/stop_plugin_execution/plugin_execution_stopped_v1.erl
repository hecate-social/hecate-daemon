...
-export([event_type/0]).
-export([get_plugin_id/1, get_oci_image/1, get_stopped_at/1]).
...
to_map(#plugin_execution_stopped_v1{} = E) ->
    #{
        ...
        oci_image  => E#plugin_execution_stopped_v1.oci_image,
    ...
    case {PluginId, OciImage} of
        ...
        _ ->
            {ok, #plugin_execution_stopped_v1{
                plugin_id = PluginId,
                oci_image = OciImage,
                stopped_at = hecate_api_utils:get_field(stopped_at, Map, erlang:system_time(millisecond))
            }}
    end.
...
-spec get_oci_image(plugin_execution_stopped_v1()) -> binary().
get_oci_image(#plugin_execution_stopped_v1{oci_image = V}) -> V.
