...
start_infra() ->
    ...
    TmpDir = "/tmp/test_dispatch_design_" ++
...
stop_infra(#{tmp_dir := TmpDir}) ->
    reckon_db_sup:stop_store(design_division_store),
    os:cmd("rm -rf " ++ TmpDir),
    ok.
...
dispatch_persists_event() ->
    DivisionId = unique_id(<<"des-persist">>),
    {ok, Cmd} = start_design_v1:new(#{division_id => DivisionId}),
    {ok, _, [_]} = maybe_start_design:dispatch(Cmd),
    {ok, Events} = esdb_gater_api:get_events(
        design_division_store, DivisionId, 0, 100, forward),
    ?assertEqual(1, length(Events)).

dispatch_returns_version_and_events() ->
    DivisionId = unique_id(<<"des-ret">>),
    {ok, Cmd} = start_design_v1:new(#{division_id => DivisionId}),
    {ok, Version, EventMaps} = maybe_start_design:dispatch(Cmd),
...
dispatch_second_command_appends_to_stream() ->
    DivisionId = unique_id(<<"des-stream">>),
    {ok, StartCmd} = start_design_v1:new(#{division_id => DivisionId}),
    {ok, _, _} = maybe_start_design:dispatch(StartCmd),
    {ok, ArchiveCmd} = archive_design_v1:new(#{division_id => DivisionId}),
    {ok, _, _} = maybe_archive_design:dispatch(ArchiveCmd),
    {ok, Events} = esdb_gater_api:get_events(
        design_division_store, DivisionId, 0, 100, forward),
    ?assertEqual(2, length(Events)).

read_back_event_data_matches() ->
    DivisionId = unique_id(<<"des-read">>),
