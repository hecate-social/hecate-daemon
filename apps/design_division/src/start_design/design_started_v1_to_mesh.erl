-module(design_started_v1_to_mesh).
-behaviour(gen_server).
-export([start_link/0, emit/1]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2]).

-define(TOPIC, <<"hecate.design.started">>).
...
handle_cast({emit, EventData}, State) ->
    ...
    case hecate_mesh_client:publish(?TOPIC, EventData) of
        ok ->
            logger:info("[design_started_v1_to_mesh] emitted for division ~s", [DivisionId]);
        {error, Reason} ->
            logger:warning("[design_started_v1_to_mesh] failed for division ~s: ~p", [DivisionId, Reason])
    end,
    {noreply, State}.
