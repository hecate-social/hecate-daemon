...
handle(Cmd, _Context) ->
    case resume_design_v1:validate(Cmd) of
        ok ->
            Event = design_resumed_v1:new(#{
                division_id => resume_design_v1:get_division_id(Cmd)
            }),
            {ok, [Event]};
        {error, _} = Err -> Err
    end.
...
dispatch(Cmd) ->
    DivisionId = resume_design_v1:get_division_id(Cmd),
    Timestamp = erlang:system_time(millisecond),
    EvoqCmd = #evoq_command{
        command_id = generate_command_id(DivisionId, Timestamp),
        command_type = resume_design,
        aggregate_type = design_aggregate,
        aggregate_id = DivisionId,
        payload = resume_design_v1:to_map(Cmd),
